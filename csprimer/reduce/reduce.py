...
def my_zip(*iters):
    ...
    def iter_zip(iterables):
        import sys
        xs = [iter(it) for it in iterables]
        while True:
            try:
                step = [next(it) for it in xs]
                # print("iter_zip", step, file=sys.stderr)
                yield step
            except StopIteration:
                # print("iter_zip", "StopIteration", file=sys.stderr)
                return

    return reduce(lambda x, y:  x + [y], iter_zip(iters), [])
