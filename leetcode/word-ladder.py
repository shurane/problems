...
import importlib
wordladderinput = importlib.import_module("word-ladder-input")
    ...
    def ladderLengthSingleBFS(self, beginWord: str, endWord: str, wordList: List[str]) -> int:
        ...
        while queue:
            ...
            if word == endWord:
                ...
                #     chain.append(curr)
                #     curr = links[curr]
                # print(list(reversed(chain)), len(chain))
                return length
...
assert s.ladderLengthSingleBFS("hit", "cog", ["hot","dot","dog","lot","log","cog"]) == 5
...
assert s.ladderLengthSingleBFS("nanny", "aloud", wordladderinput.wordlist1) == 20